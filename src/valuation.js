...
// Write a value to EVERY matching output.
// The same data-output can appear on loading, noresults and result screens.
...
export function applyOutputs(doc, { response, fullAddress, postcode, beds, leadstart }) {
  ...
  setFieldValue(
    doc.querySelector('[data-process="property_address"]'),
    fullAddress || showAddress || postcode
  );
  ...
  // Uplift vs a standard long-term let = short-term annual − long-term annual.
  const annualN = num(annual);
  const occN = num(occupancy);
  const llAnnualN = num(llAnnual);

  if (annualN && llAnnualN) {
    set('[data-output="delta-annual"]', `+${money(annualN - llAnnualN)}`);
  }

  // Derived stats from annual + occupancy.
  if (annualN) {
    ...
    if (occN) {
      set(
        '[data-output="nightly-value"]',
        money(Math.round(monthly / (30 * (occN / 100))))
      );
    }
  }
  ...
  // Hidden/form field used for HubSpot.
  // Must receive the monthly estimate range, not annual revenue.
  const estimationValue = estimateRange;
  ...
  // Force visible estimate range again in case the result screen/card updates after applyOutputs.
  function writeEstimateRange() {
    doc.querySelectorAll('[data-output="estimate-range"]').forEach((el) => {
      el.textContent = estimateRange;
    });
  }

  writeEstimateRange();
  setTimeout(writeEstimateRange, 100);
  setTimeout(writeEstimateRange, 500);
  setTimeout(writeEstimateRange, 1000);

  // Let the HubSpot bridge know the estimate is ready.
  window.dispatchEvent(
    new CustomEvent("staymo:estimate-ready", {
      detail: {
        response,
        estimationValue,
        estimateRange
  ...
  // Debug storage for checking in console.
  ...
  try {
    localStorage.setItem("latestEstimateApiResponse", JSON.stringify(response, null, 2));
    localStorage.setItem("estimateDebug", JSON.stringify(window.estimateDebug, null, 2));
  } catch (e) {}
}

// Drive the funnel transition.
// Prefers the engine's screen controller; falls back to toggling legacy data-display marker.
function goTo(doc, key) {
  ...
  if (el) {
    el.style.display = key === "result" ? "flex" : "block";
  }
}

// Show the noresults screen.
// applyOutputs never runs on this path, so write the attempted address here too.
function showNoResultsState(doc, address) {
  if (address != null) {
    writeOutputs(doc, '[data-output="address"]', address);
  }

  goTo(doc, "noresults");
}

// Draw the chart once the result screen is actually visible.
// longTerm = ll_estimate monthly long-term rent.
function drawChartWhenVisible(doc, min, max, longTerm) {
  if (!window.initChart) return;

  const screen =
    doc.querySelector("[start-result]") ||
    doc.querySelector('[data-display="result"]');
...
export function initValuation(doc = document) {
  doc.addEventListener("click", (e) => {
    const trigger =
      e.target.closest('[aria-label="Start Estimate"]') ||
      e.target.closest("[start-ready-button]");

    if (!trigger) return;

    setTimeout(() => {
      const addressEl =
        doc.querySelector("#autocompleteHero") ||
        doc.querySelector('[data-input-id="address-search"]');
      ...
      const leadstart = (
        new URL(doc.location?.href || location.href).searchParams.get("sourcepath") || ""
      ).trim();
      ...
      const body = buildRequestBody({
        ...
        postcode,
        beds,
        utm: getUTMFromURL(location.href)
      });
      ...
      fetch("https://valuation.smarthost.co.uk/smarthost-estimate.php", {
        ...
        .then((response) => {
          ...
          // Belt-and-braces update after the result screen is active.
          setTimeout(() => {
            const estimateRange =
              response?.estimate ||
